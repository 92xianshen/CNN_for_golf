...
for i in xrange(ext_signal.shape[0]):
    pred, acc = test_fn(np.asarray([ext_signal[i, opt['sig_start']:opt['sig_end']]], dtype=np.float32), np.asarray([ext_labels[i]], dtype=np.uint8))
    prediction[i] = pred
    test_acc.append(acc)
