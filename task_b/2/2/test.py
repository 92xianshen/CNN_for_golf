...
network = InputLayer(shape=(None, 3, 1000))
...
signal = np.load('X_test2.npz')['arr_0']
labels = np.load('y_test2.npz')['arr_0']
...
for i in xrange(ext_signal.shape[0]):
    pred, acc = test_fn(np.asarray([ext_signal[i, 2:5]], dtype=np.float32), np.asarray([ext_labels[i]], dtype=np.uint8))
    prediction[i] = pred
    test_acc.append(acc)
